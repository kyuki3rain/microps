...
static int ether_tap_addr(struct net_device *dev)
{
    ...
    if (soc == -1)
    {
        errorf("socket() failed: %s, dev=%s", strerror(errno), dev->name);
        return -1;
    }
    ...
    {
        errorf("ioctl SIOCGIFHWADDR failed: %s, dev=%s", strerror(errno), dev->name);
        close(soc);
        return -1;
    }
...
static int ether_tap_open(struct net_device *dev)
{
    ...
    if (tap->fd < 0)
    {
        errorf("open %s failed: %s, dev=%s", CLONE_DEVICE, strerror(errno), dev->name);
        return -1;
    }
    ...
    {
        errorf("ioctl TUNSETIFF failed: %s, dev=%s", strerror(errno), dev->name);
        close(tap->fd);
        return -1;
    }
    ...
    {
        errorf("fcntl F_SETOWN failed: %s, dev=%s", strerror(errno), dev->name);
        close(tap->fd);
        return -1;
    }
    ...
    {
        errorf("fcntl F_SETFL failed: %s, dev=%s", strerror(errno), dev->name);
        close(tap->fd);
        return -1;
    }
    ...
    {
        errorf("fcntl F_SETSIG failed: %s, dev=%s", strerror(errno), dev->name);
        close(tap->fd);
        return -1;
    }
...
static ssize_t ether_tap_read(struct net_device *dev, uint8_t *buf, size_t size)
{
    ...
    if (len < 0)
    {
        ...
        {
            errorf("read() failed: %s, dev=%s", strerror(errno), dev->name);
            return -1;
        }
    }
...
static int ether_tap_isr(unsigned int irq, void *id)
{
    ...
    while (1)
    {
        ...
        if (ret < 0)
        {
            ...
            errorf("poll() failed: %s, dev=%s", strerror(errno), dev->name);
            return -1;
        }
        if (ret == 0)
        {
...
struct net_device *ether_tap_init(const char *name, const char *addr)
{
    ...
    if (addr)
    {
        if (ether_addr_pton(addr, dev->addr) < 0)
        {
